...
class CocoDataset(Pix2pixDataset):
    ...
    def get_paths(self, opt):
        ...
        edge_dir = os.path.join(root, '%s_edge' % phase)
        edge_paths = make_dataset(edge_dir, recursive=False, read_cache=True)

        if not opt.coco_no_portraits and opt.isTrain:
            edge_portrait_dir = os.path.join(root, '%s_edge_portrait' % phase)
            if os.path.isdir(edge_portrait_dir):
                edge_portrait_paths = make_dataset(edge_portrait_dir, recursive=False, read_cache=True)
                edge_paths += edge_portrait_paths
