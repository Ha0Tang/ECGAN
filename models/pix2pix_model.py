...
class Pix2PixModel(torch.nn.Module):
    ...
    def preprocess_input(self, data):
        ...
        # print(label_map.size()) [4, 1, 256, 512]

        ### calculate existence label
        target_label_existence = torch.squeeze(label_map, dim=1)
        # print(target_label_existence.size()) [4, 256, 512]
        tvect = torch.zeros(bs, self.opt.label_nc)
        for i in range(bs):
            hist = torch.histc(target_label_existence[i].cpu().data.float(),
                               bins=self.opt.label_nc, min=0, max=self.opt.label_nc - 1)
            vect = hist > 0
            tvect[i] = vect
        # print(tvect.size()) [4, 35]
        # print(tvect)
        ### calculate existence label
        ...
        # edge_map = data['edge']
        # print(edge_map.size())　[4, 3, 256, 512]
        # input_semantics = torch.cat((input_semantics, edge_map), dim=1)
        return input_semantics, tvect, data['image'], data['edge']
    ...
    def compute_generator_loss(self, input_semantics, label_existence, real_image, real_edge):
        ...
        G_losses['GAN'] = self.criterionGAN(pred_fake, True, for_discriminator=False) + \
                          ...
                          self.criterionGAN(pred_fake2, True, for_discriminator=False) * 2
        ...
        if not self.opt.no_vgg_loss:
            G_losses['VGG'] = self.criterionVGG(fake_image, real_image) * self.opt.lambda_vgg + \
                              ...
                              self.criterionVGG(fake_image2, real_image) * self.opt.lambda_vgg * 2
    ...
    def compute_discriminator_loss(self, input_semantics, label_existence, real_image, real_edge):
        ...
        with torch.no_grad():
            ...
            fake_image2.requires_grad_()

            fake_edge = fake_edge.detach()
            fake_edge.requires_grad_()
        ...
        D_losses['D_Fake'] = self.criterionGAN(pred_fake, False, for_discriminator=True) + \
                             ...
                             self.criterionGAN(pred_fake2, False, for_discriminator=True) * 2
        D_losses['D_real'] = self.criterionGAN(pred_real, True, for_discriminator=True) + \
                             self.criterionGAN(pred_real_edge, True, for_discriminator=True) + \
                             self.criterionGAN(pred_real2, True, for_discriminator=True) * 2
